...
**What are MCP servers?** MCP (Model Context Protocol) servers extend Claude with additional tools for tasks like reading PDFs or searching the web. Each server adds a specific capability.
...
External Plugins (via Claude CLI):
  ...
  ❯ ○ document-skills - Excel, Word, PowerPoint, PDF creation/editing
    ◉ code-review-ai - AI-powered architectural review and code quality analysis (recommended)

  ↑↓ navigate  ⎵ toggle  ⏎ confirm
```

**What are external plugins?** These are third-party Claude plugins that add specialized capabilities. They're installed via Claude's built-in plugin system.

| Plugin | Pre-selected | What it does |
|--------|:---:|--------------|
| **code-review-ai** | Yes | Adds an AI architect that reviews your code changes for quality, patterns, and potential issues. Integrates into the development workflow. |
...
<Callout>
**Recommendation:** Keep **code-review-ai** selected. It's part of the recommended development workflow: write code, then get an automated review before committing.
</Callout>
...
- **CLAUDE.md** is the global configuration file that defines your workflow. It's the core of the setup.
