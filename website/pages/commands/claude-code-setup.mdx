...
| Upgrade custom | Custom repo has updates (auto-refreshes modules) |
...
External plugins (like `comprehensive-review`) are installed via the Claude CLI:
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(*install.sh --refresh-custom*)",
...
| `Bash(*install.sh --refresh-custom*)` | Phase 3 when upgrading custom |
