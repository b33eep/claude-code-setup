...
Team members update with `/claude-code-setup` — it detects new versions automatically and runs `--refresh-custom` to:
- Re-copy updated custom skills to `~/.claude/skills/`
- Re-apply custom command overrides and scripts
- Auto-discover and install new custom skills
- Auto-discover and install new custom MCP servers (if no API key required)
- Rebuild CLAUDE.md with updated module tables

## Removing a Custom Repository

To uninstall the custom modules repo entirely, run [`/remove-custom`](/commands/remove-custom) (or `./install.sh --remove-custom` from the CLI). It removes the git clone, every installed custom skill / MCP / command / script, clears the tracking fields in `installed.json`, restores base versions of overridden commands, and rebuilds CLAUDE.md.
...
- [/remove-custom](/commands/remove-custom) - Remove the active custom repo and all its artifacts
- [Module Management](/features/module-management) - `--list`, `--update`, `--remove`, `--remove-custom`
